 ...
 * @author AMIRA
 */
class OrdennanceLigneService {

    public void addOrdennanceLigne(OrdennanceLigne ordennanceLigne) {
        try {
            String req = "INSERT INTO `ordennance_ligne`(`medicamment_id`, `ordennance_id`, `quantite`) VALUES (?,?,?,?)";
    ...
    public ArrayList<OrdennanceLigne> ordennanceligneListe(int id) {
        ...
        try {
            String req = "SELECT * FROM `order_line` WHERE `related_rendezvous_id` = ?";
            PreparedStatement pst = MyConnection.getInstance().getCnx().prepareStatement(req);
            pst.setInt(1, id);
            ...
            while (rs.next()) {
                ...
                ordennanceLigne.setQuantite(rs.getInt("quantite"));
